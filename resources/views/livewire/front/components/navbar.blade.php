<nav x-data="{ open: false, scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 20)"
    :class="{
        'bg-black/20 backdrop-blur-md shadow-sm border-gray-100': scrolled,
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20 items-center">
            ...
            <!-- Desktop Menu -->
            <div
                class="hidden md:flex space-x-1 items-center bg-white/50 backdrop-blur-sm px-2 py-1.5 rounded-full border border-gray-200/50 shadow-sm">
                <a href="{{ route('home') }}" wire:navigate
                    class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ request()->routeIs('home') ? 'bg-white text-accent shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-gray-100/50' }}">
                    Home
                </a>
                <a href="{{ route('about') }}" wire:navigate
                    class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ request()->routeIs('about') ? 'bg-white text-accent shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-gray-100/50' }}">
                    About
                </a>
                <a href="{{ route('services') }}" wire:navigate
                    class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ request()->routeIs('services') ? 'bg-white text-accent shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-gray-100/50' }}">
                    Services
                </a>
                <a href="{{ route('contact') }}" wire:navigate
                    class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ request()->routeIs('portfolio') ? 'bg-white text-accent shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-gray-100/50' }}">
                    Portfolio
                </a>
                <a href="{{ route('contact') }}" wire:navigate
                    class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ request()->routeIs('contact') ? 'bg-white text-accent shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-gray-100/50' }}">
                    Contact
                </a>
            </div>
            ...
            <div class="hidden md:flex items-center">
                <a href="{{ route('contact') }}" wire:navigate
                    class="text-white bg-green-600 hover:bg-green-800 px-6 py-2.5 rounded-full transition-all text-sm font-medium shadow-md hover:shadow-lg hover:-translate-y-0.5">
            ...
            <div class="md:hidden flex items-center">
                <button @click="open = !open"
                    class="text-slate-800 hover:text-slate-600 focus:outline-none p-2 rounded-md">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-100"
        ...
        class="md:hidden absolute top-20 left-0 w-full bg-white border-b border-gray-100 shadow-lg"
        style="display: none;">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <a href="{{ route('home') }}" wire:navigate
                class="block px-3 py-2 rounded-md text-base font-medium text-slate-700 hover:text-slate-900 hover:bg-gray-50">Home</a>
            <a href="{{ route('about') }}" wire:navigate
                class="block px-3 py-2 rounded-md text-base font-medium text-slate-700 hover:text-slate-900 hover:bg-gray-50">About</a>
            <a href="{{ route('services') }}" wire:navigate
                class="block px-3 py-2 rounded-md text-base font-medium text-slate-700 hover:text-slate-900 hover:bg-gray-50">Services</a>
            <a href="{{ route('contact') }}" wire:navigate
                class="block px-3 py-2 rounded-md text-base font-medium text-accent font-semibold bg-blue-50">Contact
                Us</a>
        </div>
    </div>
</nav>
